...
bool ExpectDiscreteFeaturesSet(const auto& discrete,
  ...
  for (std::size_t index = 0; index < kTurnFeatureCount; ++index) {
    ...
    if (discrete[index] != expected_value) {
      std::cerr << "FAIL: discrete feature mismatch at index " << index
                << ", expected " << static_cast<int>(expected_value)
                << ", got " << static_cast<int>(discrete[index]) << '\n';
      ok = false;
    }
  }
...
bool ExpectMaterializedFeaturesSet(
    const neuroevolution::model::input_encoder::TurnInputVector& materialized,
    const std::initializer_list<std::size_t> expected_indices) {
  std::array<bool, kTurnFeatureCount> expected{};
  ...
  for (std::size_t index = 0; index < kTurnFeatureCount; ++index) {
    ...
    if (materialized[index] != expected_value) {
      std::cerr << "FAIL: materialized feature mismatch at index " << index
...
bool TestEncodeTurnFeaturesGoldenCase() {
  ...
  constexpr std::array<std::size_t, 10> kExpectedActiveIndices{
      ...
      GuessLetterFeatureOffset(1, 1),
      GuessLetterFeatureOffset(2, 2),
      GuessLetterFeatureOffset(3, 3),
      GuessLetterFeatureOffset(4, 4),
      FeedbackFeatureOffset(0, 0),
      FeedbackFeatureOffset(1, 1),
      FeedbackFeatureOffset(2, 2),
      FeedbackFeatureOffset(3, 0),
