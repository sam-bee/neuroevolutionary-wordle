...
private:
  static void PopulateParameters(
    ...
    parameters.input_to_hidden.weights[(0 * kTurnFeatureCount) + kGuessB1] =
        0.5f;
    parameters.input_to_hidden.weights[(0 * kTurnFeatureCount) + kGreen0] =
        4.0f;

    parameters.input_to_hidden.weights[(1 * kTurnFeatureCount) + kGuessC2] =
        2.0f;
    parameters.input_to_hidden.weights[(1 * kTurnFeatureCount) + kGrey2] =
        -5.0f;
  ...
  static void PopulateExpectedOutput(
      model::input_encoder::EncodedTurnVector &expected_output) {
