...
using device_evaluation_ops::GenomeEvaluationEpisodeTileScratch;
...
using device_evaluation_ops::TryPlayWordleEpisodeTileConcurrently;
...
__global__ void EvaluateSlabGenerationFitnessByEpisodeKernel(
    ...
    __shared__ GenomeEvaluationEpisodeTileScratch<kDynamicPolicyWarpSize, kEvaluationWarpsPerBlock> tile_scratch;
    if (!genotype_slab::IsValidGenotypeSlabLayout(slab_layout) || (slab_storage == nullptr) ||
    ...
    for (std::size_t episode_tile_base = (block_tile_index * kEvaluationWarpsPerBlock); episode_tile_base < episode_count;
        ...
        if (lane_index == 0) {
            ...
            tile_scratch.warp_has_episode[warp_index] = warp_has_episode ? 1U : 0U;
            tile_scratch.warp_active[warp_index] = warp_has_episode ? 1U : 0U;
        ...
        if (threadIdx.x == 0) {
            ...
            for (std::size_t active_warp_index = 0; active_warp_index < kEvaluationWarpsPerBlock; ++active_warp_index) {
                if ((tile_scratch.warp_has_episode[active_warp_index] != 0) &&
        ...
        const DeviceGenomeEvaluationStatusCode evaluation_status =
            TryPlayWordleEpisodeTileConcurrently<kDynamicPolicyWarpSize, kEvaluationWarpsPerBlock>(
                genome_bytes, training_word_catalog, selectable_action_count, evaluation_scratch, tile_scratch);
        if (evaluation_status != DeviceGenomeEvaluationStatusCode::kOk) {
            if (threadIdx.x == 0) {
                SetFailureStatus(status, MapEvaluationStatus(evaluation_status));
            }
        ...
        if ((lane_index == 0) && (tile_scratch.warp_has_episode[warp_index] != 0)) {
