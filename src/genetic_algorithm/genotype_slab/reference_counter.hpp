...
inline NEUROEVOLUTION_HOST_DEVICE bool IsValidBufferParentIndex(const std::uint32_t *slot_indices,
    ...
    return (slot_indices != nullptr) && (parent_index < active_individual_count) &&
           (slot_indices[parent_index] != kInvalidSlabSlotIndex);
}

inline NEUROEVOLUTION_HOST_DEVICE bool IsValidBufferParentIndex(const SlabGenerationView generation,
                                                                const std::uint32_t parent_index) noexcept {
    return IsValidSlabGenerationView(generation) &&
...
inline NEUROEVOLUTION_HOST_DEVICE bool TryBuildParentReferenceCounts(const std::uint32_t *slot_indices,
                                                                     const std::size_t active_individual_count,
                                                                     const SlabParentPair *parent_pairs,
...
inline NEUROEVOLUTION_HOST_DEVICE bool TryBuildParentReferenceCounts(const SlabGenerationView generation,
                                                                     const SlabParentPair *parent_pairs,
                                                                     const std::size_t child_count,
                                                                     std::uint32_t *parent_reference_counts) noexcept {
    if (!IsValidSlabGenerationView(generation)) {
        return false;
    }
...
TryCollectZeroReferenceParents(const GenotypeSlabView buffer, const SlabGenerationView current_generation,
                               const std::uint32_t *parent_reference_counts) noexcept {
    if (!IsValidGenotypeSlabView(buffer) || !IsValidSlabGenerationView(current_generation) ||
    ...
    for (std::size_t parent_index = 0; parent_index < current_generation.active_individual_count; ++parent_index) {
        ...
        if (!TryReleaseSlabSlot(buffer, current_generation.slot_indices[parent_index])) {
            return false;
        }
...
inline NEUROEVOLUTION_HOST_DEVICE bool TryReleaseParentReference(const GenotypeSlabView buffer,
                                                                 const SlabGenerationView current_generation,
