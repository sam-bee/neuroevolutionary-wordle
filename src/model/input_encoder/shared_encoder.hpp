...
struct SharedEncoderParameters {
    ...
    DenseLayerParameters<kEncoderHiddenSize, kEncoderOutputSize> hidden_to_output{};
};
...
inline NEUROEVOLUTION_HOST_DEVICE void ApplyReLU(EncoderHiddenVector &activations) noexcept {
    for (std::size_t activation_index = 0; activation_index < kEncoderHiddenSize; ++activation_index) {
